...
{
    // Base case: if the current word's length equals the desired size
    if (word.Length == size)
    {
        results.Add(word);
        return;
    ...
    // Recursive case: iterate through available letters
    for (int i = 0; i < letters.Length; i++)
    {
        // Choose the current letter and exclude it for the next recursion
        string newWord = word + letters[i];
        string remainingLetters = letters.Substring(0, i) + letters.Substring(i + 1);

        // Recurse with the updated word and letters
